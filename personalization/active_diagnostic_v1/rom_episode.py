"""Frozen-ROM wrapper for Active Prior Diagnostic Arbitration V1."""
...
class ROMGatedActiveDiagnosticRunResult:
    episode_id: str
    rom_profile_id: str
    rom_profile_fingerprint: str
    sequential_result: ActiveDiagnosticSequentialRunResult
...
class ROMGatedActiveDiagnosticEpisode:
    """One active-diagnostic episode bound to one immutable ROM profile."""
    ...
    def __init__(
        ...
        *,
        ...
        if not profile.frozen:
            raise RuntimeError("active diagnostic requires ROM_PROFILE_FROZEN=true")
        if domain.profile.fingerprint != profile.fingerprint:
            raise RuntimeError("active-diagnostic domain/ROM fingerprint mismatch")
        if adaptation_budget != PRIMARY_ADAPTATION_BUDGET:
            raise ValueError("active-diagnostic primary episode requires K=4")
        ...
        self.result: ROMGatedActiveDiagnosticRunResult | None = None
    ...
    def run(
        ...
        *,
        ...
        if self.status != "READY":
            raise RuntimeError(f"active-diagnostic episode is not runnable: {self.status}")
        sequential = run_active_prior_diagnostic_arbitration(
            environment,
            self.domain,
        ...
        self.result = ROMGatedActiveDiagnosticRunResult(
            episode_id=self.episode_id,
    ...
    def restart_for_changed_rom(
        ...
        *,
        ...
        if new_domain.profile.fingerprint != new_profile.fingerprint:
            raise RuntimeError("replacement active-diagnostic domain/profile mismatch")
        self.status = "CLOSED_ROM_PROFILE_CHANGED"
        self.closed_reason = "ROM_CHANGE_INVALIDATES_PERSONALIZATION_EPISODE"
        return ROMGatedActiveDiagnosticEpisode(
