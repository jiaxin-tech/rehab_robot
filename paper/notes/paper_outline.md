...
| `lower_limb_sim/config.py` uses knee ROM 5–130 deg, while current execution/reference audit uses 5–145 deg. | Legacy workspace/IK uses 5–130 deg; active reference closure audit explicitly uses 5–145 deg. Hip remains 0–120 deg in both. | Keep knee ROM experiment-configured; do not claim one repository-wide knee upper limit until harmonized. |
