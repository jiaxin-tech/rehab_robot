...
当前实现已推进到 [MyoLeg 附加弹簧—阻尼开发试验](MYOLEG_PHYSICAL_INTERACTION_PILOT_V1.md)：先纠正重复速度缩放、弹簧方向和辅助参数污染主体阻力的问题，再用同样的 27 个 V3 候选做有单位的机械配置筛查。它与下面的解析 resistance 软件压力测试分开，不能把解析测试的成功当成物理机制的成功；物理筛查通过前不进入新的算法排名。

2026-09-25 已完成上述物理筛查：3 个 native 开发主体 × 4 个机械配置、324 条响应、6 个 oracle，最大公共相对 regret 0.466442%，0/12 配置达到 0.5% 门槛，结论 HOLD。当前下一步转向测量有效性和参数可辨识性；不调大系数、不新增候选版本、不因接近门槛而改判。此前解析算法比较不能替代这一步，确认实验仍未达到开启条件。
