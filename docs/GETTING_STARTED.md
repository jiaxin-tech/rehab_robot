...
2026-09-23 本机验证：Windows x64、CPython 3.12.14，使用上述锁定依赖，`pip check` 通过。环境配置后的核心回归为 `333 passed in 62.70 s`；文档和文件整理后复跑，**37 个文件、333 项测试全部通过，耗时 64.30 s**，无失败或跳过。最新 JUnit 记录位于本地 `.cache/core-regression.xml`。该结果验证离线算法与 fake/诊断代码，不代表真机运动验证或历史全仓回归。
